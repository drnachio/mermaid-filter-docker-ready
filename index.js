...
function mermaid(type, value, format, meta) {
    ...
    var fullCmd = `${cmd}  ${confFileOpts} ${puppeteerOpts} --no-sandbox -w ${options.width} -s ${options.scale} -f -i "${tmpfileObj.name}" -t ${options.theme} -b ${options.background} -o "${savePath}"`
